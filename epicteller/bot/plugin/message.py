...
rollback = on_notice(lambda b, e, d: e.detail_type == 'group_recall')
...
async def _(bot: Bot, event: Event, state: dict):
    ...
    await rollback.finish(f"🗑️ {MessageSegment.at(event.user_id)} 消息「{content.to_message()}」已删除。")
