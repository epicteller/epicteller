...
class CredentialDAO:
    ...
    async def create_access_credential(cls, credential: Credential):
        await cls.r.pool.set(f'access_token:{credential.token}', credential.to_json(), expire=credential.ttl)

    @classmethod
    async def create_refresh_credential(cls, credential: Credential):
        await cls.r.pool.set(f'refresh_token:{credential.token}', credential.to_json(), expire=credential.ttl)
